...
public class WebOffice {
	...
	private boolean LoadFile() throws GlobalException {
		...
		if (null != mBusType && !"".equals(mBusType) && !"null".equals(mBusType))
	...
	private boolean SaveFile() throws GlobalException {
		boolean mResult = false;
		//int iFileId = -1;
		String Sql = "SELECT * FROM Doc_File WHERE BusId = '" + mBusId + "' AND BusType='" + mBusType + "' AND  RecordID='" + mRecordID + "'";
		try {
			if (null != DbaObj.Conn) {
				try {
					ResultSet result = DbaObj.ExecuteQuery(Sql);
					if (result.next()) {
						Sql = "update Doc_File set RecordID=?,FileName=?,FileType=?,FileSize=?,FileDate=?,FileBody=?,FilePath=?,UserName=?,Descript=?,BusId=?,BusType=? WHERE RecordID='" + mRecordID + "'";
					} else {
	...
	private boolean getLastFile() throws GlobalException {
		boolean mResult = false;
		String Sql = "SELECT FileBody,FileSize,RecordID FROM Doc_file WHERE RecordID = (select RecordID from lab_office_doc where Template ='" + mTemplate + "' order by FileDate desc limit 0,1) ";
		try {
			if (null != DbaObj.Conn) {
				try {
					ResultSet result = DbaObj.ExecuteQuery(Sql);
	...
	public String SaveByteToFile(String filePath, String tempId) throws GlobalException {
		//boolean mResult = false;
		String Sql = "SELECT FileBody,FileSize,fileType FROM Temp_File WHERE RecordID='" + tempId + "'";
		try {
				...
				try {
					...
					if (result.next()) {
						try {
							...
							if (!pathFile.exists())
								pathFile.mkdirs();
							filePath += File.separator + tempId + result.getString("fileType");
	...
	private boolean getTempFile() throws GlobalException {
		boolean mResult = false;
		String Sql = "SELECT FileBody,FileSize FROM Temp_File WHERE RecordID='" + mTemplate + "'";
		try {
			if (null != DbaObj.Conn) {
				try {
					ResultSet result = DbaObj.ExecuteQuery(Sql);
		...
		try {
			if (null != DbaObj.Conn) {
				try {
					ResultSet result = DbaObj.ExecuteQuery(Sql);
					if (result.next()) {
						Sql = "update Temp_File set RecordID=?,FileName=?,FileType=?,FileSize=?,FileDate=?,FileBody=?,FilePath=?,UserName=?,Descript=? WHERE RecordID='" + mTemplate + "'";
					} else {
	...
	private boolean ListVersion() throws GlobalException {
		boolean mResult = false;
		String Sql = "SELECT FileID,FileDate,UserName,Descript FROM Vsion_File WHERE RecordID='" + mRecordID + "'";
		mFileID = "\r\n";
		mDateTime = "保存时间\r\n";
		mUserName = "用户名\r\n";
		mDescript = "版本说明\r\n";
	...
	private boolean LoadVersion(String mFileID) throws GlobalException {
		boolean mResult = false;
		String Sql = "SELECT FileBody,FileSize FROM Vsion_File WHERE RecordID='" + mRecordID + "' and  FileID=" + mFileID;
		try {
			if (null != DbaObj.Conn) {
				try {
					ResultSet result = DbaObj.ExecuteQuery(Sql);
	...
	private boolean LoadBookMarks() throws GlobalException {
		boolean mResult = false;
		String Sql = " select b.BookMarkName,b.BookMarkText from Temp_BookMarks a,BookMarks b where a.BookMarkname=b.BookMarkName and a.RecordID='" + mTemplate + "'";
		try {
			if (null != DbaObj.Conn) {
				try {
					ResultSet result = DbaObj.ExecuteQuery(Sql);
	...
	private boolean SaveBookMarks() throws GlobalException {
		...
		try {
			if (null != DbaObj.Conn) {
				try {
					java.sql.PreparedStatement prestmt = null;
					String Sql = "DELETE FROM Temp_BookMarks Where RecordID='" + mTemplate + "'";
					prestmt = DbaObj.prepareStatement(Sql);
					...
					for (mIndex = 7; mIndex <= MsgObj.GetFieldCount() - 1; mIndex++) {
						...
						Sql = "insert into Temp_BookMarks (RecordId,BookMarkName) values ('" + mTemplate + "','" + mBookMarkName + "')";
	...
	//调入签名纪录
	private boolean LoadMarkImage(String vMarkName, String vPassWord) throws GlobalException {
	...
	private boolean LoadSignature() throws GlobalException {
		...
		String Sql = "SELECT MarkName,UserName,DateTime,HostName,MarkGuid FROM Doc_Signature WHERE RecordID='" + mRecordID + "'";
		mMarkName = "印章名称\r\n";
		mUserName = "签名人\r\n";
		mDateTime = "签章时间\r\n";
		mHostName = "客户端IP\r\n";
	...
	private byte[] ReadPackage(HttpServletRequest request) throws GlobalException {
		byte mStream[] = null;
		int totalRead = 0;
		int readBytes = 0;
		int totalBytes = 0;
	...
	//发送处理后的数据包
	@SuppressWarnings("unused")
	private void SendPackage(HttpServletResponse response) throws GlobalException {
		try {
	...
	public void ExecuteRun(HttpServletRequest request, HttpServletResponse response) throws GlobalException {
			...
			if (request.getMethod().equalsIgnoreCase("POST")) {
				...
				if (MsgObj.GetMsgByName("DBSTEP").equalsIgnoreCase("DBSTEP")) { //判断是否是合法的信息包，或者数据包信息是否完整
					...
					//下面的代码为创建印章列表
						...
						MsgObj.MsgTextClear();
						//清除文本信息
						if (LoadMarkList(String.valueOf(request.getSession().getAttribute("thisUserId")))) {
