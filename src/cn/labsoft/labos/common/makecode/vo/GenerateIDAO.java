...
public class GenerateIDAO {
	 ...
	 * @throws
	 */
	public static void createEntityIDAO(Packagee packagee,List<Prop> listProp,String path) throws GlobalException{
		List<String> list=new ArrayList<String>();
		List<String> copyList=FileOperater.readFileByLines(path+"\\WebRoot\\utils\\gmt\\read\\IMouldDAO.java");
		String pageSrc="cn.labsoft.labos."+packagee.getName()+".dao";
		String voSrc="cn.labsoft.labos."+packagee.getName()+".entity."+packagee.getClazz().getClazzName()+"";
		if (null != copyList && 0 < copyList.size()) {
			for (String lineStr : copyList) {
				lineStr = lineStr.replace("packSrc", pageSrc);
				lineStr = lineStr.replace("entitySrc", voSrc);
		...
		path+="\\src\\cn\\labsoft\\labos\\"+packagee.getNamePath()+"\\dao\\I"+packagee.getClazz().getClazzName()+"DAO.java";
