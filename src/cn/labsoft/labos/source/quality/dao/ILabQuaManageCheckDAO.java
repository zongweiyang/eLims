package cn.labsoft.labos.source.quality.dao;

import java.util.List;
import cn.labsoft.labos.framework.common.dao.IBaseDAO;
import cn.labsoft.labos.framework.common.exception.GlobalException;
import cn.labsoft.labos.framework.common.page.PageResult;
import cn.labsoft.labos.source.quality.entity.LabQuaManageCheck;



public interface ILabQuaManageCheckDAO extends IBaseDAO{
	/**
	 * 添加管理评审记录对象信息
	 * @param labQuaManageCheck 管理评审记录对象
	 * @return 返回类型 管理评审记录对象(LabQuaManageCheck) 
	 * @throws GlobalException
	 * @author amy
	 */
	public LabQuaManageCheck addLabQuaManageCheck(LabQuaManageCheck labQuaManageCheck)throws GlobalException;
	/**
	 * 修改管理评审记录对象信息
	 * @param labQuaManageCheck 管理评审记录对象 
	 * @return 返回类型 boolean 
	 * @throws GlobalException
	 * @author amy
	 */
	public boolean updateLabQuaManageCheck(LabQuaManageCheck labQuaManageCheck)throws GlobalException;
	/**
	 * 根据客户Id获得管理评审记录对象信息
	 * @param id 管理评审记录Id
	 * @return 返回类型 管理评审记录对象(LabQuaManageCheck) 
	 * @throws GlobalException
	 * @author amy
	 */
	public LabQuaManageCheck getLabQuaManageCheck(String id)throws GlobalException;
	/**
	 * 删除管理评审记录对象信息
	 * @param labQuaManageCheck 管理评审记录对象
	 * @return 返回类型 boolean  
	 * @throws GlobalException
	 * @author amy
	 */
	public boolean deleteLabQuaManageCheck(LabQuaManageCheck labQuaManageCheck)throws GlobalException;
	/**
	 * 根据hql获取管理评审记录对象分页信息列表
	 * @param hql 管理评审记录SQL
	 * @param pageResult
	 * @return PageResult
	 * @throws GlobalException
	 * @author amy
	 */
	public PageResult getLabQuaManageCheckPR(String hql, PageResult pageResult)throws GlobalException;
	/**
	 * 根据hql获取管理评审记录对象信息列表
	 * @param hql 管理评审记录SQL
	 * @return 返回类型 管理评审记录对象集合(List) 
	 * @throws GlobalException
	 * @author amy
	 */
	public List<LabQuaManageCheck> getLabQuaManageCheckByHQL(String hql)throws GlobalException;
}