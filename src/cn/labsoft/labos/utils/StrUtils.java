...
public class StrUtils {
	 ...
	 * 
	 */
	public static String encode(String inputStr) throws GlobalException {
		...
		if (null != acronymPos && 0 < acronymPos.size()) {
			...
			for (int i = 0; i < acronymPos.size(); i++) {
				if (acronymPos.get(i) != 0 && i != acronymPos.size() - 1) {
					colStr += "_"
							+ camelStr.substring(acronymPos.get(i), acronymPos
									.get(i + 1));
	...
	public static byte[] objectToByteArray(Object obj) throws GlobalException {
		...
		try {
			...
			//e.printStackTrace();
			Log4J.err(e.getMessage());
			throw new GlobalException("" + e.getMessage());
		}
		return bytes.clone();
	}

	public static Object byteArrayToObject(byte[] bytes) throws GlobalException {
...
//		
//		List<BarCodeContent> xxx = (List<BarCodeContent>)byteArrayToObject(bytes);
//		
