...
{
	   return validateRequired(form)&& validateDateTime(form)&& validateInteger(form)  && validateDate(form)&&validateFloat(form) && validateMaxLength(form) && validateEmail(form);
}
...
function validateDateTime(form) {
   ...
   var i = 0;
   var fields = new Array();
   oDate = new DateTimeValidations();
   for (x in oDate) {
       var value = form[oDate[x][0]].value;
       //����??��2??��o??��2???DT??
       //var datePattern = oDate[x][2]("datePatternStrict");
       var dateValue=value.substring(0,value.indexOf(" "));
       var timeValue=value.substring(value.indexOf(" ")+1);
       var datePattern ="yyyy-MM-dd";
       ...
       if ((form[oDate[x][0]].type == 'text' ||
            form[oDate[x][0]].type == 'textarea') &&
            value.length>0&&
           (dateValue.length > 0) &&(timeValue.length > 0) &&
           (datePattern.length > 0)&&
         ...
         if ((orderMinute < orderSecond && orderMinute > orderHour)) {
             ...
             var matched = timeRegexp.exec(timeValue);
             if(matched!=null){
...
function addTime(obj){
 today=new Date();
 timestr=today.toLocaleTimeString();
 if(obj.value.length<11)
 obj.value=obj.value+" "+timestr;
}
