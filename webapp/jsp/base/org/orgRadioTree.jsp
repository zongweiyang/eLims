...
html {
	...
	var api = frameElement.api, W = api.opener, D = W.document; // api.opener 为载加lhgdialog.min.js文件的页面的window对象
		...
		function submitvalueforlist(){
			...
			funIds="";
			if(nodes.length==1){
				funIds=nodes[0].treeNid;
				$('#labOrgVo_parentId').val(funIds);
			}else{
				alert('<s:property value="getText('select.transition')"/>');
				return ;
			}
			var df = document.listForm;
			df.action=basePath+"/org/labOrg/updateLabOrgMove.action";
		    df.submit();
		}
						...
						<tr>
							<td>
								...
								<div style="top: 95px; left: 135px; PADDING-BOTTOM: 5px; PADDING-LEFT: 50px; PADDING-RIGHT: 0px; PADDING-TOP: 5px; overflow: auto; _height: 430px; min-height: 430px;">
									<ul id="zTree" turl="${basePath}/org/labOrg/treeRadioLabOrg.action?labOrgVo.id=${labOrgVo.id}&labOrgVo.parentId=${labOrgVo.parentId}" radio="true" radioType="all"></ul>
								</div>
							</td>
						</tr>
					</table>
