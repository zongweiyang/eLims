		...
		<script language="JavaScript">
			function submitvalue(actionstr){
		  	...
		  	function doAction(page){
		  		...
		  		$("#pages").val(pageNum);
		  		submitvalue('/user/labUser/listUserQuery.action');
		  	}
		  	function addRow(obj){
		  		obj=$(obj);
						...
						success:function (data){
							...
							var html='<input type="hidden" value="'+data.nameChin+'" name="labQueryVo.listQuery['+rowIndex+'].demo"  />';
				   	 		...
				   	 		if(data.isVague!=null&&data.isVague=='1'){
				   	 			...
				   	 			likeHtml+='<option value="LIKE"><s:property value="getText('mohu.query')"/></option>';
				   	 			likeHtml+='<option value="="><s:property value="getText('jingque.query')"/></option>';
				   	 			likeHtml+='</select>';
			   	 			...
			   	 			if(data.isSort!=null&&data.isSort=='1'){
				   	 			...
				   	 			orderHtml+='<option value="ASC"><s:property value="getText('up.order')"/></option>';
				   	 			orderHtml+='<option value="DESC"><s:property value="getText('down.order')"/></option>';
				   	 			orderHtml+='</select>';
		  	...
		  	function showValue(index){
					...
					var url  = '${basePath}/query/labQuery/preDiaLog4Select.action';
						$.dialog({
							id:'ids',
							content:'url:'+url,
																...
																<s:iterator value="#alllist" status="st">	
																	<tr>
																		<td width="100"> <label><s:text name="query.state"/>：</label></td>
								                      						...
								                      						<td width="50">
								                      							<s:if test="${isSort=='1'}">
								                      								<s:select name="labQueryVo.listQuery[${st.index}].isSortValue" theme="simple" value="'${isSortValue}'" list="#{'1':getText('selected.pls'),'ASC':getText('up.order'),'DESC':getText('down.order')}"></s:select>
													...
													<td class="blockTd"
														...
														<table cellspacing="0" cellpadding="0" border="0">
															<tr>
																...
																<td>
																		<td><a id="BtnPreview" class="zPushBtn" href="javascript:void(0);" onclick="submitvalue('/user/labUser/listUserQuery.action'); return false;"><img height="20" width="20" src="${basePath}img/chakan.gif"/><b><s:text name="fun.query"/></b></a></td>
																</td>
															</tr>
														</table>
													</td>
