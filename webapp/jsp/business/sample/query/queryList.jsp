		...
		<script language="JavaScript">
			function submitvalue(actionstr){
		  	...
		  	function doAction(page){
		  		...
		  		$("#pages").val(pageNum);
		  		submitvalue('/sample/labSampRegister/labSampRegisterQuery.action');
		  	}
																		...
																		<s:iterator status="st" value="#alllist" >
																			...
																			<td>
																				...
																				<s:if test="${showType}==0">
																						<input type="text" value="${value}" size="23" name="labQueryVo.listQuery[${st.index}].value"  />
																				</s:if>
																				<s:if test="${showType}==1">
																						<input type="text" name="labQueryVo.listQuery[${st.index}].value" value="${value}" class="Wdate" size="23" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true});" />
																				</s:if>
																				<s:if test="${showType}==2">
																						<input type="text" name="labQueryVo.listQuery[${st.index}].value" value="${value}" class="Wdate" size="23" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true});" />
																				</s:if>
													...
													<td class="blockTd"
														...
														<table cellspacing="0" cellpadding="0" border="0">
															<tr>
																...
																<td>
																		<td><a id="BtnPreview" class="zPushBtn" href="javascript:void(0);" onclick="submitvalue('/sample/labSampRegister/labSampRegisterQuery.action'); return false;"><img height="20" width="20" src="${basePath}img/chakan.gif"/><b><s:text name="fun.query"/></b></a></td>
																</td>
															</tr>
														</table>
													</td>
								...
								<tr>
									<td align="center" class="tou" colspan="5">
										<div class="pagecontrol">
											<s:text name="page.current"/>${labQueryVo.pageVo.pages+1}<s:text name="page.page"/>|&nbsp;<s:text name="page.total"/><s:text name="page.total"/>${labQueryVo.pageVo.countPages}<s:text name="page.page"/>&nbsp;
											<s:if test="${labQueryVo.pageVo.pages}==0">
												<s:text name="page.first"/>
											</s:if>
											<s:else>
												<a href="javascript:;" onclick="doAction('start');"><s:text name="page.first"/></a>
											</s:else>
											&nbsp;
											<s:if test="${labQueryVo.pageVo.pages}==0">
