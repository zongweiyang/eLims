		...
		function copy(obj){
			...
			var cIndex=obj.closest('td').index();
				$(".myworkingboxttable tr:gt(0)").find("input[type*=checkbox]").each(function(){
					if($(this).attr("checked")){
			    		$(this).closest('tr').find('td').eq(cIndex).find("select").val(obj.val());
		    		}
		...
		function showTestInfo(){
			...
			$(".myworkingboxttable tr:gt(0)").each(function(){
				if($(this).find("input[type=checkbox]").attr("checked")){
					...
					itemNames+=",";
					itemSamNum+=$(".myworkingboxttable tr").eq(rIndex).find('td').eq(5).find('input').val();
					itemSamNum+=",";
					testMan+=$(".myworkingboxttable tr").eq(rIndex).find('td').eq(6).find('select').val();
					testMan+=",";
					taskIds+=$(".myworkingboxttable tr").eq(rIndex).find('td').eq(6).find('input').val();
					taskIds+=",";
					checkMan+=$(".myworkingboxttable tr").eq(rIndex).find('td').eq(7).find('select').val();
					checkMan+=",";
				}
			});
			if(itemNames.length>0)itemNames=itemNames.substr(0,itemNames.length-1);
			itemNames=encodeURI(encodeURI(itemNames));
									...
									<table class="myworkingboxttable" id="orgTable">
										...
										<s:iterator value="#request.listLabSamTestVo" status="st">
											<tr>
												...
												<td class="c">
													<input type="text" style="width: 45px;" value="${sampNum-sampNumTask}" name="listLabSamTestVo[${st.index}].sampNum" onchange="checkSampNum('${sampNum-sampNumTask}',this)" />
													<font color="red">*(<s:text name="assign.number"></s:text>)</font>
												</td>
												<td style="text-align: center;" onclick="checkTd(this)">
													<input type="hidden" id="labSamTaskId" value="${taskId}" />
