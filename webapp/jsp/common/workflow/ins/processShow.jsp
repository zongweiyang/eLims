...
<body onload="loadDefault();" style="overflow:hidden;">
...
<div style="float:left;width:100%;height:${labWfProcessInsVo.viewHeight}px;position:relative;"  >
	<div id="canvas" class="canvas"  style="width:100%;height:100%;text-align: right;padding-top:10px;padding-right: 10px;border: 1px;" >
		${labWfProcessInsVo.processName}
	</div>
</div>
 ...
 */
 var NodeType={
 	Node:'Node',Start:'Start',End:'End',Link:'Link'
 }
  /**
 * <pre>
 * 	画布（工作区）大小和位置 
...
Component.prototype=
{
	...
	*/
	...
	{
		var othis=this;
		ExtModalDialog.show("节点属性","<%=basePath%>/jsp/workflow/temp.jsp",window,refreshDoc.bind(othis),{width:500,height:230},{'node':othis,'man':componentMan});
	},
	/**
	*格式
	*/
...
Node.prototype.dblfun=function()
{
	...
	ExtModalDialog.show("节点属性","<%=basePath %>workflow/process/showLabWfStep.action?labWfStepVo.operType="+othis.attObj.objectextend.objecttemplate+"&labWfStepVo.id="+this.id+"&labWfStepVo.name="+encodeURIComponent(_value)+"&labWfStepVo.ids="+othis.attObj.objectextend.views_type+"&labWfStepVo.overDate="+overDate+"&labWfStepVo.url="+url,window,refreshDoc.bind(othis),{width:300,height:300});
}
/*
 *	回调函数设置节点值
 */
...
Link.prototype=
{
	...
	*/
	...
	dblfun:function()
	{
		...
		var busId='${labWfProcessInsVo.id}';
		ExtModalDialog.show("线路属性","showLabWfPath.action?labWfPathVo.name="+encodeURIComponent(_value)+"&labWfPathVo.ids="+encodeURIComponent(othis.attObj.basic.name),window,refreshDoc.bind(this),{width:500,height:230},othis);
	},
	//获得拐点之前的直线
    ...
    initFromXML:function(doc)
    {
        ...
        for (var i = 0; i < nodeList.length; i++)
        {
            ...
            if(type==NodeType.Node)
            {
                ...
                status = status.trim();
                if(status=='0'){
                	n.changePicgif("unrunTaskNode.png");
                }else if(status=='1'){
                	n.changePicgif("running.gif");
                }else if(status=='2'){
                	n.changePicgif("runningTaskNode.png");
                }else if(status=='3'){
               		n.changePicgif("Node.png");
                }else{
                	n.changePicgif("Node.png");
                }
            }
             if(type==NodeType.ChildWorkFlow)
            {
                ...
                n.updateText(n.attObj.basic.name);
                if(status=='0'){//实例初始
                	n.changePicgif("ChildWorkFlow_init.png");
                }else if(status=='1'){//实例预处理
                	n.changePicgif("ChildWorkFlow_run.png");
                }else if(status=='2'){ //实例进行中
                	n.changePicgif("ChildWorkFlow_run.png");
                }else if(status=='3'){//实例完成
               		n.changePicgif("ChildWorkFlow.png");
                }else{
                	n.changePicgif("ChildWorkflow.png");
                }
            }
        ...
        for(var i=0;i<linkList.length;i++)
        {
            ...
            $("link_text"+id).parentNode.style.top=parseInt(texttop);
           	...
           	l.noSelect();
            l.updateText(l.attObj.basic.desc);
        }
...
var File={
	...
	{
		  var doc = new ActiveXObject("Microsoft.XMLDOM");
		  if('${labWfProcessInsVo.flowXml}'!=''){
		  	doc.loadXML('${labWfProcessInsVo.flowXml}');
		  }else{
			doc.loadXML('');  
		  }
