...
.ChildWorkFlow_cur
{
	cursor:url(<%=basePath %>workflowskins/default/images/workflow/cur/ChildWorkFlow.cur);
}
</style>
...
<div id="testdiv" style="border:1px solid #000;display:none;width:500px;height:500px;position:absolute;z-index:9000;left:0px;top:0px;background:red;"></div>
<script>
...
<div style="float:left;width:100%;height:440px;position:relative"  >
...
ChildWorkFlowAtt.prototype={
	afterInit:function()
	{
		this.basic.childworkflowbsid='';
	},
	afterXML:function(doc,att)
	{
		var basic=att.selectSingleNode("BASIC");
		basic.setAttribute("CHILDWORKFLOWBSID",this.basic.childworkflowbsid);
	},
    afterinitattfromxml:function(node)
    {
        var basic=node.selectSingleNode("ATT/BASIC");
        this.basic.childworkflowbsid=basic.getAttribute("BUSINESSCODE");
    }
}
...
Component.prototype=
{
	initialize:function(position,type,id)
	{
		...
		//this.element.style.background="url(img/big/test.gif)  no-repeat";
	...
	*/
	...
	addlink:function(link)
	{
		...
		//document.debug.document.write(this.links.length+':<br />');
	},
	/**
	*节点拖动的函数
	*/
 	  ...
 	  $A(this.links).each(function(value)
 	  		{
 	  			if(value.isStartOrEnd(othis)=='start')
 	  			{
					...
					//document.debug.document.write(value.points[1].x+",");
					var newp=othis.getlineCrossRect(p,value.points[1]);
					//document.debug.document.write(newp.x+",");
	...
	*/
	...
	{
		var othis=this;
		ExtModalDialog.show("<div align='left'>节点属性</div>","<%=basePath%>/jsp/workflow/temp.jsp",window,refreshDoc.bind(othis),{width:600,height:400,maximizable: false,collapsible: false},{'node':othis,'man':componentMan});
	},
	/**
	*格式
	*/
...
Node.prototype.dblfun=function()
{
	...
	ExtModalDialog.show("<div align='left'>节点属性</div>","<%=basePath %>workflow/process/showLabWfStep.action?labWfStepVo.operType="+othis.attObj.objectextend.objecttemplate+"&labWfStepVo.id="+this.id+"&labWfStepVo.name="+encodeURIComponent(_value)+"&labWfStepVo.ids="+othis.attObj.objectextend.views_type+"&labWfStepVo.overDate="+overDate+"&labWfStepVo.url="+url,window,refreshDoc.bind(othis),{width:600,height:400,maximizable: false,collapsible: false});
}
/*
 *	回调函数设置节点值
 */
function callBackNodeFun(stepName,overDate,type,ids) {
...
function callBackLineFun(pathName,pathstr) {
	othis.attObj.basic.desc = pathName;
	othis.attObj.basic.name = pathstr;
...
OrFork.prototype.dblfun=function()
{
	var othis=this;
//	alert('1237');
	ExtModalDialog.show("<div align='left'>节点属性</div>","workflow/attribute.spr?action=orfork_index",window,refreshDoc.bind(othis),{width:600,height:400},{'node':othis,'man':componentMan});
}
/**
*OrJoin
*/
...
{
	var othis = this;
	var id=othis.attObj.basic.childworkflowbsid;
	var name=othis.attObj.basic.name;
	var window = new Ext.Window({
        title:'<div align="left">'+name+'</div>', 
        width: 800, 
        height:400, 
        //布局的方式 
        layout: 'fit', 
        ...
        minimizable:false, 
        maskDisabled:true, 
        html: '<iframe  width="100%" height="100%" src="<%=basePath %>workflow/process/showLabWfProcess4Flow.action?labWfProcessVo.id='+id+'&labWfProcessVo.initStatus=${labWfProcessVo.initStatus}"></iframe>' 
    });
    window.show();
}
/**
...
Link.prototype=
{
	initialize:function(startNode,endNode,points,id)
	{
		...
		//$('link_text'+this.id).onchange=function(){othis.attObj.basic.desc=$('link_text'+othis.id).value;}; 
	...
	*/
	...
	dblfun:function()
	{
		...
		ExtModalDialog.show("<div align='left'>线路属性</div>","showLabWfPath.action?labWfPathVo.name="+encodeURIComponent(_value)+"&labWfPathVo.ids="+encodeURIComponent(othis.attObj.basic.desc),window,refreshDoc.bind(this),{width:600,height:400,maximizable: false,collapsible: false},othis);
	},
	//获得拐点之前的直线
...
var File={
	/*save:function()
	{
		var url="<%=basePath%>workflow/process/updateFlowDef.action";
		xml=componentMan.saveAsXML();
		var busId='${labWfProcessVo.id}';
        var pars="flowXml="+xml+"&busId="+busId;
		new Ajax.Request(  
		url,
		{
			method:'post',
