...
html {
       				...
       				for(var i=0;i<result.length;i++){
       					$('#bzhxx').append(
       						$('<tr>').append($('<td>').append('<input type="hidden" name="labWfProcessVo.funStepList['+i+'].stepId" value="'+result[i].id+'" id="stepId'+i+'" />'+
       								 ...
       								 .append($('<td>').append('<input readonly="readonly" type="text" name="labWfProcessVo.funStepList['+i+'].stepCode" value="'+result[i].wfcode+'" id="stepCode'+i+'" size="40"  style="background-color: #DDDDDD;"/>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:;" onclick="deleteOne(this)"><s:text name="lab.code.del"/></a>'))
       					);
       				}
				},
				error:function(){
	...
	<body class="" id="mainid">
		<form method="post" name="form" action="updateLabWfProcess.action">
			<s:token></s:token>
													...
													<td class="blockTd" style="padding: 6px 10px; vertical-align: center;">
														<table cellspacing="0" cellpadding="0" border="0">
															<tr>
																...
																<td>
																	<l:a uri="workflow/process/updateLabWfProcess.action" onclick="submitforform();" img="/img/add.gif" value="lab.code.save" />
																</td>
															</tr>
														</table>
													</td>
										...
										<div class="Formtable subWf" style="display: none;">
											...
											<table class="FormtableCon_line" id="subProcess">
												...
												<s:iterator id="" status="st" value="labWfProcessVo.subProcessList">
													<tr key="${st.index}">
														<td>
															<s:select list="#request.processList" listKey="id" listValue="name" name="labWfProcessVo.subProcessList[${st.index}].id" id="subProcessId${st.index}" theme="simple"></s:select>
														</td>
														<td>
															<input type="radio" name="labWfProcessVo.subProcessList[${st.index}].isStop" value="N" <s:if test="${isStop=='N'}">checked="true"</s:if>/><s:text name="norm.flow"/>
															<input type="radio" name="labWfProcessVo.subProcessList[${st.index}].isStop" value="Y" <s:if test="${isStop=='Y'}">checked="true"</s:if> /><s:text name="stop.wait"/>
														</td>
														<td class="c">
															<a href="javascript:;" onclick="deleteOne(this);"><s:text name="lab.code.del"/></a>
														</td>
										...
										<div class="Formtable">
											...
											<table class="FormtableCon_line" id="bzhxx">
												...
												<s:iterator id="" status="st" value="labWfProcessVo.funStepList">
													<tr>
														<td>
															<input type="hidden" name="labWfProcessVo.funStepList[${st.index}].stepId" value="${stepId}" id="stepId${st.index}" />
															<input readonly="readonly" type="text" name="labWfProcessVo.funStepList[${st.index}].stepName" value="${stepName}" id="stepName${st.index}" size="40" style="background-color: #DDDDDD;" />
														</td>
														<td>
															<input readonly="readonly" type="text" name="labWfProcessVo.funStepList[${st.index}].stepCode" value="${stepCode}" id="stepCode${st.index}" size="40" style="background-color: #DDDDDD;" />
															&nbsp;&nbsp;&nbsp;&nbsp;
															<a href="javascript:;" onclick="deleteOne(this)"><s:text name="lab.code.del"/></a>
														</td>
													</tr>
	...
	<script>
		$(function(){
			var checked='${labWfProcessVo.isSubWf}';
			if(checked=='Y'){
				$('.subWf').removeAttr('style');
			}else{
				$('.subWf').attr('style','display:none');
