							...
							<s:form theme="simple" action="" method="post" name="labAmbientInfoForm">
								...
								<div class="TabTable">
								 	<div class="FUNCIONBARNEW">
				                      ...
				                      <td><a id="BtnPreview" class="zPushBtn" href="javascript:;" onclick="submitvalue('updateLabAmbientInfo.action');return false;"><img height="20" width="20" src="<%=basePath%>img/add.gif"/><b><s:text name="lab.code.save"/></b></a></td>
				                      </tr></table></td></tr>
				                      </table>
			                    	</div>
									<div class="Formtable">
										...
										<table class="FormtableCon">
											<tr>
												...
												<td>
													<s:select list="#request.listLabAmbientVo" cssStyle="width:130px" onchange="check(this)"  headerValue="------请选择------" headerKey="0"   theme="simple" listValue="name" value="'${labAmbientInfoVo.labAmbientId}'+'|'+'${labAmbientInfoVo.labAmbientUnit}'" listKey="id+'|'+unit" ></s:select>
												</td>
												<td ><label><s:text name="param.unit"/>：</label></td>
												<td>
													<input type="text" readonly="readonly"  id="unit" value="${labAmbientInfoVo.labAmbientUnit}" />
													<input type="hidden" readonly="readonly" name="labAmbientInfoVo.labAmbientId" value="${labAmbientInfoVo.labAmbientId}" id="labAmbientId" />
												</td>
											</tr>
											<tr>
												<td><label><s:text name="param.value"/>：</label></td>
												<td><input type="text" name="labAmbientInfoVo.value" id="value" value="${labAmbientInfoVo.value}" /><font color="red">&nbsp;&nbsp;*</font></td>
												<td><label><s:text name="address"/>：</label></td>
												<td><input type="text" name="labAmbientInfoVo.address" id="address" value="${labAmbientInfoVo.address}" /></td>
											</tr>
											<tr>
												<td><label><s:text name="room"/>：</label></td>
												<td><input type="text" name="labAmbientInfoVo.room" id="room" value="${labAmbientInfoVo.room}" /></td>
												<td><label><s:text name="env.time"/>：</label></td>
												<td><input type="text" name="labAmbientInfoVo.time" id="time" value="${labAmbientInfoVo.time}" class="Wdate" size="20"  onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',alwaysUseStartDate:true});" /></td>
											</tr>
											<tr>
												<td>
