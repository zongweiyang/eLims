...
<html>
  ...
  <SCRIPT type="text/javascript">
		var setting = {
			async: {
				enable: true,
				url:"<%=basePath %>/appara/ajaxLabApparaZtree.action",
				autoParam:["treeNid"],
				dataFilter: filter
			},
			view: {expandSpeed:"",
		...
		function zTreeOnClick(event,treeId,treeNode) {
		    $('#treeMain',parent.document).attr({'src':'<%=basePath %>/appara/listLabAppara.action?labApparaVo.typeId='+treeNode.treeNid});
		};
		...
		function beforeRename(treeId, treeNode, newName) {
			...
			$.ajax({
				url:'${basePath}/appara/ajaxLabAppType4Update.action',
				type:'POST',
		...
		function addHoverDom(treeId, treeNode) {
			...
			if (btn) btn.bind("click", function(){
				$.ajax({
					url:'${basePath}/appara/ajaxLabAppType4Add.action',
					type:'POST',
