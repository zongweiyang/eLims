...
<html>
	...
	<SCRIPT type="text/javascript">
		var setting = {
			async: {
				enable: true,
				url:"<%=basePath%>consumables/labConsumables/ajaxLabConsumablesZtree.action",
				autoParam:["treeNid"],
				dataFilter: filter
			},
			view: {expandSpeed:"",
		...
		function zTreeOnClick(event,treeId,treeNode) {
		    $('#treeMain',parent.document).attr({'src':'<%=basePath%>consumables/labConsumables/listLabConsumables.action?labConsumablesVo.consumablesTypeId='+treeNode.treeNid});
		};
		...
		function beforeRemove(treeId, treeNode) {
			...
			if(confirm("确认删除分类 " + treeNode.name + " 吗？")){
				$.ajax({
					url:'${basePath}consumables/labConType/ajaxNode4Delete.action',
					type:'POST',
		...
		function beforeRename(treeId, treeNode, newName) {
			...
			$.ajax({
				url:'${basePath}/consumables/labConType/ajaxNode4Update.action',
				type:'POST',
		...
		function addHoverDom(treeId, treeNode) {
			...
			if (btn) btn.bind("click", function(){
				$.ajax({
					url:'${basePath}/consumables/labConType/ajaxNode4Add.action',
					type:'POST',
