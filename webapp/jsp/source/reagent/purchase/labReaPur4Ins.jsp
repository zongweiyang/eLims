	...
	_overflow-x: hidden;
		...
		<script>
			...
			function ajax2LabReaList(){
				var url  = '${basePath}reagent/labReaPurMain/ajax4ListLabPur.action';
				$.dialog({
					id:'roleId',
					content:'url:'+url,
					title:'试剂列表',
		    ...
		    function reLoadPage(ids){
			   	var url='reagent/labReaPurMain/showLabReaPur4Ins.action?labReaPurMainVo.reagentId='+ids;
			   	$('form').attr('action','${basePath}'+url);
				$('form').submit();
		   }
		</script>
															...
															<tr>
																...
																<td>
																	<a id="BtnPreview" class="zPushBtn" href="javascript:;"
																		onclick="javascript:if(confirm('确认要提交吗?')){goAction('updateLabReaPur4Ins.action?labReaPurMainVo.auditResult=1');}return false;"><img
																			height="20" width="20"
													...
													<s:iterator value="reaPurDetailList" status="st">
														<tr>
															<td class="c">
																...
																<input type="hidden" name="reagentId"
																	value="${reagentId}" />
															</td>
															<td class="c">
																${reagentName }
