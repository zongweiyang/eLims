...
html {
	_overflow-x: hidden;
		...
		<script>
			...
			var df=document.labRefPutInsform;
			df.action = actionstr;
			df.submit();  
		}
			...
			dataType:'text',
	  		success:function (data){
	   	  		if(data != "false"){
	   	  			window.location.href = '<%=basePath%>reference/labRefPurMain/'+actionstr+'&path='+data;
				}
			  }
	          });
    	   }
	...
	<body class="" id="mainid">
		<s:form theme="simple" action="" method="post" name="labRefPutInsform">
						...
						<table class="workingBody" cellspacing="0" cellpadding="0"
													...
													<td class="blockTd"
														...
														<table cellspacing="0" cellpadding="0" border="0">
															<tr>
																...
																<td>
																	<l:a uri="reference/labRefPurMain/preAddLabRefPurIns.action"
																		value="admin.add" />
																</td>
															</tr>
														</table>
														...
														<tr>
															...
															<td class="c">
																<s:if test="${isOper=='Y'&& type=='1'}">
																	<l:a href="#"
																		uri="reference/labRefPurMain/preUpdateLabRefPurIns.action?labRefPurMainVo.id=${id}"
																		value="lab.code.modify" />
																	&nbsp;&nbsp;
																		 <l:a href="#"
																		uri="reference/labRefPurMain/deleteLabRefPurIns.action?labRefPurMainVo.id=${id}"
																		 onclick="deleteOne('reference/labRefPurMain/deleteLabRefPurIns.action?labRefPurMainVo.id=${id}');" value="lab.code.del" />
																</s:if>
																<s:elseif test="${isOper=='Y'&& type=='0'}">
																	<l:a href="#"
																		uri="reference/labRefPurMain/showLabRefPur4Ins.action?labRefPurMainVo.id=${id}"
																		value="inbound.con" />
																</s:elseif>
																<s:else>
																	<l:a href="#"
																		...
																		value="look.check" />
																	<a href="#"
																		onclick="ajaxVerification('exportLabRefPur.action?labRefPurMainVo.id=${id}');return false;"><s:text name="indoundbill"/></a>
																</s:else>
															</td>
														</tr>
													</s:iterator>
							...
							<tr>
								<td align="center"><jsp:include
										page="/jsp/include/page.jsp?formName=labRefPutInsform"
										flush="true" /></td>
							</tr>
						</table>
					</td>
