...
<%@ page import="cn.labsoft.labos.source.reference.vo.LabReferenceVo"%>
...
<%
	...
	java.util.List<LabReferenceVo> oldNoList=(java.util.List<LabReferenceVo>)request.getAttribute("labReferenceVoList");
	int size=oldNoList.size();
	int index=0;
	for(LabReferenceVo vo:oldNoList){
		name.append(vo.getName()+",");
		guige.append(vo.getSize()+"$");
		saveOrgName.append(vo.getSaveOrgName()+"|");
		address.append(vo.getExt01()+"*");
	}
...
var size = parseInt('<%=size%>');
    ...
    {
		document.refReferencetypeForm.action = actionstr;
		document.refReferencetypeForm.submit();  
	}
	</script>
		<style>
.bodytable .oRight {
...
html {
	...
	<body class="" id="mainid" style="overflow-y: auto; height: 100%">
		<form action="" method="post" name="refReferencetypeForm">
									...
									<div class="myworkingbox">
										...
										<div id="printDiv">
											<table class="myworkingboxttable" cellspacing="1"
												cellpadding="0">
												<s:if test="labReferenceVoList!=null">
													<s:set name="alllist" value="labReferenceVoList" />
													<s:iterator value="#alllist" status="st">
														...
														<tr>
															<td>
																<img
																	src="<%=basePath%>jsp/source/reference/barCode/${id}.png"
																	alt="二维码"
...
function printlnTable() {
	...
	function CreftePrintPage() {
		LODOP=getLodop(document.getElementById('LODOP'),document.getElementById('LODOP_EM')); 
		
		LODOP.SET_PRINT_PAGESIZE(1,"80mm","55mm","CrefteCustomPage"); 
		...
		for(i=0;i<size;i++){
			var html = '<div style="border:1px solid;height:173px;width:290px;"><table cellspacing="0" cellpadding="0" style="margin:0;padding:0;"><tr><td align="center" style="height:20px;line-height:8px;font-size:12px;font-weight:bold;font-family:仿宋_GB2312;vertical-align:middle;width:100px;valign-align:middle;">标准品名称：</td><td style="height:20px;font-size:12px;width:230px;line-height:8px;font-weight:bold;font-family:仿宋_GB2312;vertical-align:middle;border-bottom:1px dashed #ccc;">'+name.split(",")[i]+'</td></tr>';
				...
				html+='<tr><td align="center" style="height:30px;line-height:10px;font-size:12px;font-weight:bold;font-family:仿宋_GB2312;vertical-align:middle;"></td><td style="height:47px;line-height:8px;font-size:12px;font-weight:bold;font-family:仿宋_GB2312;vertical-align:middle;border-bottom:1px dashed #ccc;"><img width="70px" src="<%=basePath%>/jsp/source/reference/barCode/'+address.split("*")[i]+'.png" /></td></tr>';
				html+='</table></div>';
			LODOP.ADD_PRINT_HTM("10px","-3px","400px","180px",html);
			LODOP.NEWPAGE();
		}
	};	
