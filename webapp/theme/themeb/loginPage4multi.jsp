...
function checkUser() {
	var msg = "";
	if($("#tenantName").val()==''){
			msg+='<s:property value="getText('theme.input.username')"/>'+'请输入租户名!\n';
			alert(msg);
			$("#tenantName").focus();
	}else{
		if($("#loginname").val() ==''){
			msg+='<s:property value="getText('theme.input.username')"/>'+'请输入用户名!\n';
			alert(msg);
			$("#loginname").focus();
		}else{
			if($("#password").val() ==''){
				msg+=+'<s:property value="getText('theme.input.password')"/>'+'请输入密码!\n';
				alert(msg);
				$("#password").focus();
			}else{
				if($("#checkcode").val() ==''){
					msg+='<s:property value="getText('theme.input.checkma')"/>'+'请输入验证码!\n';
					alert(msg);
					$("#checkcode").focus();
				}
			}
