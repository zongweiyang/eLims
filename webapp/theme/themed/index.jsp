...
<script type="text/javascript" language="JavaScript" src="<%=basePath%>js/jquery-1.3.2.js"></script>
...
<table border="0" cellpadding="1" cellspacing="1" width="260" style="margin:0 auto;margin-top:120px;">
  <tbody><tr>
...
<tr>
  <td colspan="2" align="center"><p id="msgTxt" style="color:#F00;">登陆中...</p></td>
</tr>
...
<tr>
  ...
  <td>
  <input type="text" id="loginname" name="sysUserVo.loginname" style="border: 1px solid #000;height:20px;line-height:20px;width:128px;font-sie:12px;padding-left:5px;" class="gaia le val" >  </td>
</tr>
...
<tr>
  ...
  <td>
  <input id="password" name="sysUserVo.password" value="" style="border: 1px solid #000;height:20px;line-height:20px;width:128px;font-sie:12px;padding-left:5px;" class="gaia le val" type="password">  </td>
</tr>
<tr>
...
<script type="text/javascript" language="JavaScript">
	var check;
	function eLogin() {
	window.setTimeout(function(){document.all.msgTxt.innerHTML="登陆中...";},2000);
		try{
			//测试e盾驱动和安全控件安装是否正确
			var test = CertControl.TestControl();
			if (!test) {
				window.setTimeout(function(){document.all.msgTxt.innerHTML="请正确安装e盾驱动！";},2000);
				return false;
			}
			//测试设备是否连接电脑
			var isConn = CertControl.IsKeyConnect();
			if (isConn) {
				window.setTimeout(function(){document.all.msgTxt.innerHTML="检查到e盾已连接，请稍候..."},2000);
				//设备初始化
				var initRes = CertControl.InitKey();
				window.setTimeout(function(){document.all.msgTxt.innerHTML="正在初始化e盾，请稍候...";},2000);
				if (!initRes) {
					window.setTimeout(function(){document.all.msgTxt.innerHTML="e盾初始化失败！";},2000);
					return false;
				}else{
					window.setTimeout(function(){document.all.msgTxt.innerHTML="e盾初始化成功！";},2000);
				}
				//读取设备序列号
				var uksn = CertControl.GetDeviceSN();
				window.setTimeout(function(){document.all.msgTxt.innerHTML="正在读取e盾序列号，请稍候...";},2000);
				if (uksn == "") {
					window.setTimeout(function(){document.all.msgTxt.innerHTML="读取e盾序列号失败！";},2000);
					return false;
				}else{
					window.setTimeout(function(){document.all.msgTxt.innerHTML="读取e盾序列号成功！";},2000);
				}
				//读取签名证书
				var cert = CertControl.GetSignCert();
				window.setTimeout(function(){document.all.msgTxt.innerHTML="正在得到e盾签名证书，请稍候...";},2000);
				if (cert == "") {
					window.setTimeout(function(){document.all.msgTxt.innerHTML="得到e盾签名证书失败！";},2000);
					return false;
				}else{
					window.setTimeout(function(){document.all.msgTxt.innerHTML="得到e盾签名证书成功！";},2000);
				}
				//访问签名服务器
				var result = CertControl.OnlineVerifySign(cert, uksn);
				window.setTimeout(function(){document.all.msgTxt.innerHTML="正在访问签名服务器...";},2000);
				//释放控件资源
				CertControl.KeyDisconnect(); 
				if(result == 0){
					window.setTimeout(function(){document.all.msgTxt.innerHTML="请检查您的网络连接是否正常！";},2000);
					return false;
				}else{
					...
					function callBackFunction(returnData){
						if(returnData == '0'){
							...
							window.setTimeout(function(){document.all.msgTxt.innerHTML="您未绑定e盾到系统，请联系管理员!";},2000);
							return;
						}else{
							window.setTimeout(function(){document.all.msgTxt.innerHTML="e盾权限认证成功，请输入系统密码!";},2000);
			...
			window.setTimeout(function(){document.all.msgTxt.innerHTML="请检查您是否安装安全控件！";},2000);
		}
	}
