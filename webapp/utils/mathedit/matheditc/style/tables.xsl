...
<xsl:template match="mml:mtd[@columnspan]">
...
<!--    this test valid for Sablotron, another form - test="not(position()=last())".
	Also for mml:mtd[@columnspan] and mml:mtr  -->
		<xsl:text>&amp; </xsl:text>
	</xsl:if>
</xsl:template>
...
<xsl:template match="mml:mtable">
	...
	<xsl:variable name="numbercols" select="count(./mml:mtr[1]/mml:mtd[not(@columnspan)])+sum(./mml:mtr[1]/mml:mtd/@columnspan)"/>
	<xsl:choose>
